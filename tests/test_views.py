...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_only__books_in_list(self):
        login = self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('my-borrowed'))
        ...
        #Check that now we have  books in the list
        resp = self.client.get(reverse('my-borrowed'))
        #Check our user is logged in
        self.assertEqual(str(resp.context['user']), 'testuser1')
        #Check that we got a response "success"
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_logged_in_with_permission__book(self):
    ...
    def test_logged_in_with_permission_another_users__book(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('renew-book-librarian', kwargs={'pk':self.test_bookinstance1.pk,}) )
    ...
    def test_redirects_to_all__book_list_on_success(self):
        login = self.client.login(username='testuser2', password='12345')
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.post(reverse('author_create'), {'first_name':'John111', 'last_name':'Chris', 'date_of_birth':'01/01/1965','date_of_death':'01/01/2016'} )
        self.author = Author.objects.get(first_name = 'John111')
        self.assertRedirects(resp, reverse('author-detail', kwargs={'pk':self.author.pk,}))
